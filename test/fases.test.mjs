...
test('caso normal: listo para EPI', () => {
  assert.equal(determinarFase(base, { knowledge }).estado, 'listo');
  assert.equal(determinarFase(base, { knowledge }).variante, 'conclusion_con_epi');
});

test('oposición → fuera de alcance (incidente)', () => {
  const e = copia(base); e.tramite.oposiciones = [{ acreedor: 'X' }];
  assert.equal(determinarFase(e, { knowledge }).estado, 'fuera_de_alcance');
});
...
test('solicitud de administración concursal o auto complementario → fuera de alcance', () => {
  ...
  assert.equal(determinarFase(a, { knowledge }).estado, 'fuera_de_alcance');
  assert.equal(determinarFase(b, { knowledge }).estado, 'fuera_de_alcance');
});
...
test('frontera de decisión humana: sin decisión o sin buena fe confirmada no se redacta', () => {
  ...
  for (const e of [a, b, c]) assert.equal(determinarFase(e, { knowledge }).estado, 'pendiente_decision');
});

test('sin traslado a los acreedores → pendiente de trámite', () => {
  const e = copia(base); e.tramite.traslado_acreedores = false;
  assert.equal(determinarFase(e, { knowledge }).estado, 'pendiente_tramite');
});
