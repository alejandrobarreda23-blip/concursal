...
test('crédito público AEAT/TGSS: tramos del art. 489.1.5.º', () => {
  assert.equal(exoneracionPublicaLimitada(400000), 400000);   // 4.000 € → íntegro
  assert.equal(exoneracionPublicaLimitada(500000), 500000);   // 5.000 € → íntegro
  assert.equal(exoneracionPublicaLimitada(800000), 650000);   // 8.000 € → 5.000 + 1.500
  assert.equal(exoneracionPublicaLimitada(1500000), 1000000); // 15.000 € → tope 10.000
  assert.equal(exoneracionPublicaLimitada(2500000), 1000000); // 25.000 € → tope 10.000
});
...
test('garantía real: solo se exonera lo que excede del valor de la garantía', () => {
  const r = clasificarCreditos([{ id: 'H', acreedor: 'Banco', concepto: 'Hipoteca', importe: 90000, clase: 'garantia_real', valor_garantia: 60000 }]);
  assert.equal(r.filas[0].exonerado, 3000000);
  assert.equal(r.filas[0].no_exonerado, 6000000);
  const r2 = clasificarCreditos([{ id: 'H', acreedor: 'Banco', concepto: 'Hipoteca', importe: 50000, clase: 'garantia_real', valor_garantia: 60000 }]);
...
test('clases no exonerables quedan íntegras fuera de la exoneración', () => {
  for (const clase of ['alimentos', 'rc_extracontractual', 'rc_delito', 'salarios', 'multa_sancion', 'costas_epi', 'publico_otro']) {
    const r = clasificarCreditos([{ id: 'X', acreedor: 'A', concepto: 'c', importe: 100, clase }]);
