...
export function redactar(ir) {
  ...
  const organo = [cab.tribunal, cab.seccion, cab.plaza ? `Plaza n.º ${cab.plaza}` : null].filter(Boolean).join('. ');
  doc.push({ tipo: 'encabezado', texto: `${organo}${cab.denominacion_historica ? ` (${cab.denominacion_historica})` : ''}` });
  doc.push({ tipo: 'encabezado', texto: `${cab.procedimiento}${cab.nig ? ` — N.I.G.: ${cab.nig}` : ''}` });
  ...
  doc.push({ tipo: 'parrafo', texto: `${cab.localidad ? `${cab.localidad}, ` : ''}${cab.fecha_larga}.` });
